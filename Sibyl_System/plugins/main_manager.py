...
async def scan(event, flags):
    ...
    await event.reply("Connecting to EDITH - X for a cymatic scan.")
...
async def revive(event):
    ...
    if not (
        ...
        return
    await a.edit("Revert request sent to EDITH - X. This might take 10minutes or so.")
...
async def bancodes(event):
        await event.send_message("HERE ARE THE BAN CODES FOR EDITH -X :
